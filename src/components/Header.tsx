...
export function Header() {
  ...
  useEffect(() => {
    if(navigator.geolocation){
      navigator.geolocation.getCurrentPosition(showPosition as unknown as PositionCallback)
    }

    async function showPosition(position: { coords: { latitude: string; longitude: string; }; }) {
    ...
    <nav className="flex items-center justify-between py-8">
      ...
      <div className="flex items-center gap-3">
        <div className="bg-purple-100 flex py-3 px-2 rounded-md gap-1">
          ...
          <p className="text-purple-800">{neighborhood}, {uf}</p>
        </div>
